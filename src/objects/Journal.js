...
export default class Journal extends Phaser.Sprite {
  ...
  setPosition( tileX, tileY ) {
    ...
    const offsetX = ( cornerX === 'WEST' ) ? ( COMPUTER_WIDTH / 2 ) : TILE_SIZE - ( COMPUTER_WIDTH / 2 );
    const offsetY = ( cornerY === 'NORTH' ) ? ( COMPUTER_HEIGHT / 2 ) : TILE_SIZE - ( COMPUTER_HEIGHT / 2 );
  ...
  enableJournal() {
    ...
    let sensorOffsetX = ( TILE_SIZE - COMPUTER_WIDTH ) / ( ( cornerX === 'WEST' ) ? 2 : -2 );
    let sensorOffsetY = ( TILE_SIZE - COMPUTER_HEIGHT ) / ( ( cornerY === 'NORTH' ) ? 2 : -2 );

    if ( cornerY === 'SOUTH' ) {
      this.body.angle = 180;
      sensorOffsetX += ( TILE_SIZE - COMPUTER_WIDTH ) * ( ( sensorOffsetX < 0 ) ? 1 : -1 );
      sensorOffsetY += ( TILE_SIZE - COMPUTER_HEIGHT ) * ( ( sensorOffsetY < 0 ) ? 1 : -1 );
    }

    const rectangleSensor = this.body.addRectangle( TILE_SIZE, TILE_SIZE, sensorOffsetX, sensorOffsetY );
